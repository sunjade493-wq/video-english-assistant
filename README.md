...
V2.5A 是 V2.4A Obstacle Timeline Frozen 之后的下一阶段设计冻结候选方案。本节只记录产品方向与交互候选，不表示已实现、已冻结或已发布。
...
因此，V2.5A 的核心指标是：剩余障碍数。

### 明确不做传统学习系统

V2.5A 不走传统生词本 / 复习系统路线，明确不做：
...
原因：这些系统不符合当前产品路线。本产品当前关注的是障碍减少、听懂度提升，以及攻克视频内容，而不是构建永久词汇掌握、背诵或复习体系。
...
例如初始识别出 100 个障碍。用户点击 `✓ 不用管我了` 后，表示该障碍在本集中已处理；它不表示永久掌握，不加入生词本，也不加入复习计划。剩余障碍数随本集已处理障碍减少，例如：
...
原 `恢复全部` 重命名为 `重置本集学习进度`。原因是该操作的实际含义是清空本集学习记录，让本集所有障碍重新出现。它不是高频操作，因此不应放在主界面。
...
- Learning Tips 卡片内保留 `✓ 不用管我了`。
- Learning Tips 卡片内新增 `↶ 撤回上一步`。
- 主界面移除 `恢复全部`。
- `⋯` 菜单内提供 `重置本集学习进度`。
