...
const ANALYZE_INPUT_PATH = path.join(DRAFT_DIR, 'p0_4a_analyze_input_smoke.json');
const OBSTACLE_DRAFT_PATH = path.join(DRAFT_DIR, 'p0_4a_obstacles_smoke_draft.json');
...
const DEFAULT_SMOKE_START = 12;
const DEFAULT_SMOKE_END = 16;
...
function parseSmokeRange(argv) {
  const range = { start: DEFAULT_SMOKE_START, end: DEFAULT_SMOKE_END };
  for (const arg of argv) {
    const match = arg.match(/^--(start|end)=(.+)$/);
    if (!match) {
      throw new Error(`Invalid argument: ${arg}. Supported arguments: --start=<integer> --end=<integer>`);
    }
    const value = Number(match[2]);
    if (!Number.isInteger(value)) {
      throw new Error(`Invalid --${match[1]} value: ${match[2]}. Expected an integer.`);
    }
    range[match[1]] = value;
  }
  if (range.start > range.end) {
    throw new Error(`Invalid smoke range: --start (${range.start}) must be <= --end (${range.end}).`);
  }
  return range;
}

function selectSmokeSubtitles(subtitles, range) {
  return subtitles.filter((subtitle) => subtitle.subtitleIndex >= range.start && subtitle.subtitleIndex <= range.end);
}
...
function buildAnalyzeInput(pilotSubtitles, learnerLevel, smokeRange) {
  return {
...
function buildPrompt(analyzeInput) {
  return {
    ...
    requiredOutput: {
      ...
      model: 'OpenAI model name used for generation',
...
function normalizeObstacleDraft(parsed, analyzeInput, model) {
  const output = {
...
async function callAi(prompt, config) {
  ...
  return {
    envelope,
    rawResponse: envelope.choices?.[0]?.message?.content || '',
    ok: response.ok,
...
async function main() {
  ...
  const smokeRange = parseSmokeRange(process.argv.slice(2));
  ...
  const pilotSubtitles = selectSmokeSubtitles(selectPilotSubtitles(allSubtitles), smokeRange);
  const learnerLevel = process.env.P0_4A_LEARNER_LEVEL || DEFAULT_LEARNER_LEVEL;
  const analyzeInput = buildAnalyzeInput(pilotSubtitles, learnerLevel, smokeRange);
  writeJson(ANALYZE_INPUT_PATH, analyzeInput);
  console.log(`Wrote analyze input: ${path.relative(REPO_ROOT, ANALYZE_INPUT_PATH)} (${analyzeInput.items.length} subtitles, range ${smokeRange.start}-${smokeRange.end})`);
  ...
  if (!config) {
    writeJson(path.join(DEBUG_DIR, 'response_envelope.json'), { skipped: true, reason: 'missing OpenAI configuration' });
    fs.writeFileSync(path.join(DEBUG_DIR, 'raw_response.txt'), '', 'utf8');
    writeJson(path.join(DEBUG_DIR, 'parsed_response.json'), null);
    fs.writeFileSync(path.join(DEBUG_DIR, 'parse_error.txt'), 'AI configuration missing: set OPENAI_API_KEY and OPENAI_MODEL (or P0_4A_ANALYZE_MODEL) to generate draft obstacles.\n', 'utf8');
  ...
  try {
    ...
    if (!aiResponse.ok) throw new Error(`OpenAI API error ${aiResponse.status}: ${aiResponse.rawEnvelopeText}`);
    const parsed = extractJson(rawResponse);
    writeJson(path.join(DEBUG_DIR, 'parsed_response.json'), parsed);
    fs.writeFileSync(path.join(DEBUG_DIR, 'parse_error.txt'), '', 'utf8');
    const normalized = normalizeObstacleDraft(parsed, analyzeInput, config.model);
