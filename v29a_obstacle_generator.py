...
COMPREHENSION_PATTERNS: List[Dict[str, str]] = [
    {"text": "Can you believe", "literal": "你能相信吗", "actual": "表示惊讶、难以置信，相当于“你敢信吗？”", "grammar": "Can you believe + 从句/名词短语；反问式口语开场，用来强化情绪。"},
    {"text": "I can't believe", "literal": "我不能相信", "actual": "我真不敢相信；强调说话人惊讶或不满。", "grammar": "I can't believe + 从句/宾语；否定情态动词表达强烈主观反应。"},
    {"text": "According to tradition", "literal": "根据传统", "actual": "按习俗来说；引出某个传统规定或社会惯例。", "grammar": "According to + 名词；介词短语作状语，说明信息依据。"},
    {"text": "hang the bedsheets outside", "literal": "把床单挂到外面", "actual": "指按传统展示床单，语境里带有婚俗和尴尬的喜剧效果。", "grammar": "hang + 宾语 + 地点副词；祈使/不定式结构中常省略主语。"},
    {"text": "we consummated", "literal": "我们完成了", "actual": "委婉地说夫妻圆房、完成婚姻关系。", "grammar": "consummate 可作及物或不及物动词；过去式表示已完成的行为。"},
    {"text": "where we're starting our honeymoon", "literal": "我们开始蜜月的地方", "actual": "说明蜜月旅程从哪里开始，where 引导地点从句。", "grammar": "where + 主语 + be + -ing；现在进行时表示计划中的近未来安排。"},
    {"text": "I suppose you're right", "literal": "我假设你是对的", "actual": "我想你说得对；带有让步、勉强同意的语气。", "grammar": "I suppose + 宾语从句；弱化语气，比 I think 更犹豫。"},
    {"text": "the perfect metaphor for", "literal": "……的完美隐喻", "actual": "用某物来非常贴切地象征另一件事。", "grammar": "metaphor for + 名词/动名词；for 标记隐喻指向的对象。"},
    {"text": "interlock with a satisfying snap", "literal": "带着令人满意的咔哒声互锁", "actual": "形容两个部件严丝合缝地扣上，声音让人觉得到位。", "grammar": "interlock with + 名词短语；with 表示伴随方式或伴随结果。"},
    {"text": "While you were sleeping", "literal": "当你正在睡觉的时候", "actual": "趁你睡着时；说明另一件事发生的时间背景。", "grammar": "While + 主语 + 过去进行时；背景动作与主句动作同时发生。"},
    {"text": "ordered room service", "literal": "点了客房服务", "actual": "在酒店叫了送餐/客房服务。", "grammar": "order + 名词；order 在餐饮场景中表示“点餐/订购”。"},
    {"text": "you thought it was going to be food", "literal": "你以为它将会是食物", "actual": "制造反差：听者以为叫的是吃的，但实际不是。", "grammar": "thought + 宾语从句；was going to be 表示过去视角下的将来。"},
    {"text": "our whole universe was in a hot, dense state", "literal": "我们的整个宇宙处于一个热而密集的状态", "actual": "大爆炸理论开场，描述早期宇宙高温高密度状态。", "grammar": "be in a/an + 形容词 + state；state 表示抽象状态。"},
    {"text": "expansion started", "literal": "膨胀开始了", "actual": "指宇宙开始扩张。", "grammar": "名词 + 不及物动词过去式；简洁叙述事件发生。"},
    {"text": "That all started with", "literal": "那一切都开始于", "actual": "说明一连串事情的起点或原因。", "grammar": "start with + 名词/动名词；with 引出起点、开端或触发因素。"},
    {"text": "get all this alone time", "literal": "得到所有这些独处时间", "actual": "获得这么多二人/独处时间，语气通常带期待或调侃。", "grammar": "get + 名词短语；all this 修饰不可数/抽象时间，强调数量。"},
    {"text": "marital congress", "literal": "婚姻大会/婚姻性交", "actual": "非常正式或滑稽地指夫妻性行为。", "grammar": "形容词 marital 修饰名词 congress；congress 在此为委婉旧式用法。"},
    {"text": "a hot, dense state", "literal": "一个热而密集的状态", "actual": "描述物理状态，不是日常“热闹”的意思。", "grammar": "冠词 + 并列形容词 + 名词；逗号连接同级形容词。"},
    {"text": "with a satisfying snap", "literal": "伴随着令人满意的咔哒声", "actual": "用声音说明动作完成得干脆、到位。", "grammar": "with + 名词短语作方式/伴随状语。"},
    {"text": "whole universe", "literal": "整个宇宙", "actual": "强调整体范围，常见于科学解释或夸张表达。", "grammar": "whole 放在名词前表示“整个的”，常与 the/our 连用。"},
]
...
def read_subtitle_rows(path: Path) -> List[SubtitleRow]:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as file_obj:
        ...
        for index, row in enumerate(reader, start=1):
            ...
            source_en = field_value(row, ["source_en", "english", "en", "text_en", "subtitle_en", "sentence_en", "original_en", "v28_en", "line_en"])
            source_zh = field_value(row, ["source_zh", "chinese", "zh", "cn", "text_zh", "subtitle_zh", "sentence_zh", "translation", "translation_zh", "v28_zh", "line_zh"])
            ...
            if start or end or source_en or source_zh:
                rows.append(SubtitleRow(start=start, end=end, source_en=normalize_text(source_en), source_zh=normalize_text(source_zh), row_index=index))
        return rows
...
def blank_obstacle(row: SubtitleRow, obstacle_type: str, priority: int, text: str) -> Dict[str, object]:
    return {"start": row.start, "end": row.end, "type": obstacle_type, "priority": priority, "text": text, "word": "", "phonetic": "", "translation": "", "literal": "", "actual": "", "grammar": "", "source_en": row.source_en, "source_zh": row.source_zh}
    ...
    for row in rows:
        ...
        for entry, pattern in compiled:
            ...
            if pattern.search(row.source_en):
                ...
                obstacle.update({"word": entry["word"], "phonetic": entry["phonetic"], "translation": entry["translation"]})
    ...
    for row in rows:
        ...
        for pattern in COMPREHENSION_PATTERNS:
            ...
            if phrase_matches(text, row.source_en):
                ...
                obstacle.update({"literal": pattern["literal"], "actual": pattern["actual"], "grammar": pattern["grammar"]})
                obstacles.append(obstacle)
    return obstacles


def sort_obstacles(obstacles: List[Dict[str, object]]) -> List[Dict[str, object]]:
    return sorted(obstacles, key=lambda item: (int(item["priority"]), parse_sortable_time(str(item.get("start", ""))), str(item.get("text", "")).lower()))


def write_json(path: Path, obstacles: List[Dict[str, object]]) -> None:
    payload = {"version": VERSION, "input": str(INPUT_CSV), "obstacle_count": len(obstacles), "obstacles": obstacles}
...
def validate_rule_libraries() -> None:
    required_vocab = {"considering", "official", "tradition", "consummated", "appropriate", "metaphor", "marital", "congress", "interlock", "satisfying", "ordered", "universe", "dense", "expansion", "autotrophs", "neanderthals", "developed", "pyramids", "unraveling", "mystery", "honeymoon"}
    ...
    required_patterns = {"can you believe", "i can't believe", "according to tradition", "hang the bedsheets outside", "we consummated", "where we're starting our honeymoon", "i suppose you're right", "the perfect metaphor for", "interlock with a satisfying snap", "while you were sleeping", "ordered room service", "you thought it was going to be food", "our whole universe was in a hot, dense state", "expansion started", "that all started with", "get all this alone time"}
